...
module EnabledModulePatch
  ...
  module InstanceMethods
    ...
    def module_enabled_with_scrum
      if name == 'scrum'
        max = 0
        ActiveRecord::Base.connection.execute("select max(position) from issues where project_id =  #{self.project_id}").each{|row| max = row[0]}
        ActiveRecord::Base.connection.execute "update issues set position = #{max} + id where position is null"
